...
([agent/adapters/mlx/backend.py](../agent/adapters/mlx/backend.py)) — the old `eot_token` getattr was
...
**M3.5 status: in tree, opt-in.** [nodes/stt/continuous.py](../nodes/stt/continuous.py)
...
| [agent/llm/node.py](../agent/llm/node.py) | Owns history; streams `llm.token` deltas; publishes cleaned reply on `llm.done`. Caps history at `MAX_HISTORY_TURNS = 8` pairs. `clean_for_tts()` ported. |
| [nodes/tts/node.py](../nodes/tts/node.py) | Real `KPipeline`. Synth thread + play thread. Sentence-streams. Cancellable. Publishes `mic.pause`, `tts.audio_chunk`, `tts.done`. |
| [agent/orchestrator.py](../agent/orchestrator.py) | Single bus consumer; state machine; spawns LLM thread per turn; **LLM-gate token buffer** (`<ignore>` suppresses TTS, `<reply>` forwards the tail). |
...
1. ✅ **`REQUIRE_WAKE_WORD = False`** in [config.py](../config.py). The
   ...
   ([nodes/stt/two_pass.py](../nodes/stt/two_pass.py)) — every phrase becomes a turn.
...
1. ✅ **Port** [always_listening_hybrid_phrase_word_pipeline.py](../../MockingAgent/PywisperCpp/pywhispercpp_examples/llm_listener/always_listening_hybrid_phrase_word_pipeline.py)
   to [nodes/stt/continuous.py](../nodes/stt/continuous.py). Same node
...
5. **Gemma 4 in `mlx-lm`** doesn't stop on `<end_of_turn>` by default —
   ...
   [agent/adapters/mlx/backend.py:stream_chat()](../agent/adapters/mlx/backend.py) that buffers the
...
7. **Whisper warm-up needs ≥1000 ms of audio**; we use 1.5 s of silence
   in [nodes/stt/two_pass.py:_warm_stt()](../nodes/stt/two_pass.py) and
   [nodes/stt/continuous.py](../nodes/stt/continuous.py). Sub-1000-ms warm
