...
**Shipped status:**
- ✅ `nodes/stt/two_pass.py` — VAD two-pass, default for `STT_MODE = "two_pass"`.
- ✅ `nodes/stt/continuous.py` — hybrid phrase/word, opt-in via `STT_MODE = "continuous"` (M3.5).
...
memory) remain unported. They can each become a `nodes/stt/*.py` node with the
